...
def keep_function(atom, data, neighbor_list):  # where keep==True
    data[int(atom[0])][5] = 0
    f = open("debug.txt", 'a')
    f.write("keep:True for atom %d %d \n" % (atom[0], atom[1]))
    for neighbor in neighbor_list:
        data[neighbor][5] = 0
        f.write("now atom %d status changed to %d: \n" %
                (neighbor, data[neighbor][5]))
    for i in range(0, data.shape[0]):
        if data[i][5] == 2:
            data[i][5] = 0
            f.write("now atom %d changed to status: %d \n" %
                    (data[i][0], data[i][5]))
    f.close()
    return data
...
def delcluster_function(s, atom, data, neighbor_list):
    data[int(atom[0])][5] = s
    f = open("debug.txt", 'a')
    f.write("keep: False, visited: False for atom %d %d \n" %
            (atom[0], atom[1]))
    for neighbor in neighbor_list:
        data[neighbor][5] = s
        f.write("now atom %d status changed to %d: \n" %
...
def determine(atom, data, neighbor_list):
    ...
    f = open("debug.txt", 'a')
    f.write("keep: False, visited: True for atom %d %d \n" %
            (atom[0], atom[1]))
    data[int(atom[0])][5] = 2
    # print data
    nextlist = []
...
def main(data):
    ...
    for i in range(0, l.shape[0]):
        ...
        f = open("debug.txt", 'a')
        f.write("neighbor list for atom %d %d \n" % (atom[0], atom[1]))
